...
public abstract class RectangleRoofTypeBuilder extends AbstractRoofTypeBuilder implements RoofTypeBuilder {
    ...
    public List<Vector3dc> createRectangle(Vector2dc[] rectangleContour) {
        List<Vector3dc> rect = new ArrayList<>(4);
        rect.add(new Vector3d(rectangleContour[0].x(), 0, -rectangleContour[0].y()));
        rect.add(new Vector3d(rectangleContour[1].x(), 0, -rectangleContour[1].y()));
        rect.add(new Vector3d(rectangleContour[2].x(), 0, -rectangleContour[2].y()));
        rect.add(new Vector3d(rectangleContour[3].x(), 0, -rectangleContour[3].y()));
        return rect;
    }
     ...
     * @return
     */
    public Vector2dc[] rectToList(RectanglePointVector2d contour) {
        Vector2dc p1 = contour.getPoint();
        Vector2d p2 = new Vector2d(contour.getVector()).mul(contour.getWidth()).add(contour.getPoint());

        Vector2d ort = new Vector2d(-contour.getVector().y * contour.getHeight(), contour.getVector().x * contour.getHeight());
     ...
     * @return roof hook space
     */
    public static PolygonRoofHooksSpace buildRecHookSpace(Vector2dc p1, Vector2d v1, PolygonPlane pPolygonPlane) {
