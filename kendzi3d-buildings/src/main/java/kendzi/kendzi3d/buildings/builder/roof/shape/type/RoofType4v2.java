...
public class RoofType4v2 extends RectangleRoofTypeBuilder {
    ...
    protected RoofTypeOutput build(PolygonWithHolesList2d buildingPolygon, double recHeight, double recWidth,
        ...
        Vector2d rightTop = new Vector2d(recWidth, recHeight);
        Vector2d rightMiddleTopPoint = new Vector2d(recWidth - l2, recHeight - l2);
        Vector2d rightCenterPoint = new Vector2d(recWidth - halfRecHeight, halfRecHeight);
        Vector2d rightMiddleBottomPoint = new Vector2d(recWidth - l2, l2);
        Vector2d rightBottom = new Vector2d(recWidth, 0);

        Vector2d leftTop = new Vector2d(0, recHeight);
        Vector2d leftMiddleTopPoint = new Vector2d(l2, recHeight - l2);
        Vector2d leftCenterPoint = new Vector2d(halfRecHeight, halfRecHeight);
        Vector2d leftMiddleBottomPoint = new Vector2d(l2, l2);
        ...
        Vector3d nt = new Vector3d(0, l2, -h2).normalize();
        Vector3d nmt = new Vector3d(0, recHeight * 0.5d - l2, -(h1 - h2)).normalize();

        Vector3d nmb = new Vector3d(0, recHeight * 0.5d - l2, h1 - h2).normalize();
        Vector3d nb = new Vector3d(0, l2, h2).normalize();

        Vector3d nr = new Vector3d(h2, l2, 0).normalize();
        Vector3d nmr = new Vector3d(h1 - h2, recHeight * 0.5d - l2, 0).normalize();
        Vector3d nl = new Vector3d(-h2, l2, 0).normalize();
        Vector3d nml = new Vector3d(-(h1 - h2), recHeight * 0.5d - l2, 0).normalize();
        ...
        Vector3d planeLeftBottomPoint = new Vector3d(leftMiddleBottomPoint.x, h2, -leftMiddleBottomPoint.y);

        Vector3d planeRightTopPoint = new Vector3d(rightMiddleTopPoint.x, h2, -rightMiddleTopPoint.y);
    ...
    public static MultiPolygonList2d intersectionOfLeftSideOfMultipleCuts(MultiPolygonList2d polygons, Vector2dc... lines) {
        return PolygonSplitHelper.intersectionOfLeftSideOfMultipleCuts(polygons,
                PolygonSplitHelper.polygonalChainToLineArray(lines));
    }
