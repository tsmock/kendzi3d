...
public class CompassDrawer {
    ...
    public void draw(Vector3dc point) {
        ...
        MatrixMath.glTranslated(point.x(), point.y(), point.z());

        double camDistanceRatio = 0.07d;
        int section = 8;

        double length = 2d * camDistanceRatio;
        double arrowLength = 0.7d * camDistanceRatio;
        double baseRadius = 0.05d * camDistanceRatio;
        double arrowRadius = 0.2d * camDistanceRatio;

        GL11.glColor3fv(Y_AXIS_COLOR_ARRAY);
        ArrowDrawUtil.drawArrow(length, arrowLength, baseRadius, arrowRadius, section);

        MatrixMath.glRotated(-90d, 0d, 0d, 1d);
