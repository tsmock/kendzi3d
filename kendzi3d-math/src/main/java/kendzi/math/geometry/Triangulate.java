...
public class Triangulate {
    ...
    /** Log. */
    private static final Logger log = LogManager.getLogger(Triangulate.class);
    ...
    private static void logBadPolygon(List<Vector2dc> contour) {
        String msg = " Bad polygon triangulation failed: \n";
        for (Vector2dc point2d : contour) {
            msg += "points.add(new Vector2d(" + point2d.x() + ", " + point2d.y() + "); \n";
        }

        log.error(msg);
    }

    public List<Integer> processIndex(List<? extends Vector2dc> pContour) {
        ...
        if (size > 1) {
            ...
            if (pContour.get(0).equals(pContour.get(pContour.size() - 1))) {
                ...
                log.error("triangulation error start and end point is doubled");
                size--;
            }
        }
