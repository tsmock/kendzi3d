...
public class RoofType2v8 extends RectangleRoofTypeBuilder {
    ...
    protected RoofTypeOutput build(PolygonWithHolesList2d buildingPolygon, double pRecHeight, double pRecWidth,
        ...
        Vector2d rightTopPoint = new Vector2d(pRecWidth, pRecHeight);
        Vector2d rightBottomPoint = new Vector2d(pRecWidth, 0);

        Vector2d leftTopPoint = new Vector2d(0, pRecHeight);
        Vector2d leftBottomPoint = new Vector2d(0, 0);
        ...
        Vector3d roofTopLineVector = new Vector3d(leftTopPoint.x - rightBottomPoint.x, 0, -(leftTopPoint.y - rightBottomPoint.y));

        Vector3d roofTopPointVector = new Vector3d(rightTopPoint.x - leftTopPoint.x, h2, -(rightTopPoint.y - leftTopPoint.y));

        Vector3d nb = roofBottomPointVector.cross(roofBottomLineVector, new Vector3d()).normalize();

        Vector3d nt = roofTopPointVector.cross(roofTopLineVector, new Vector3d()).normalize();

        Vector3d planeRightTopPoint = new Vector3d(rightTopPoint.x, middleLineHeight + h2, -rightTopPoint.y);

        Vector3d planeLeftBottomPoint = new Vector3d(leftBottomPoint.x, middleLineHeight + h1, -leftBottomPoint.y);
