...
public class Wall extends AbstractWayModel {
    ...
    private void buildWallModel(List<Vector2dc> points, double width, MeshFactory meshBorder) {
        ...
        Vector2d segmentVec = new Vector2d(segmentEnd).sub(segmentStart).normalize();
        ...
        // int pf1it = meshBorder.addVertex(new Vector3dc(pe1.x, max_height,
        // pe1.y));
        // int pf2it = meshBorder.addVertex(new Vector3dc(pe2.x, max_height,
        // pe2.y));
        // int pf1ib = meshBorder.addVertex(new Vector3dc(pe1.x, min_height,
        // pe1.y));
        // int pf2ib = meshBorder.addVertex(new Vector3dc(pe2.x, min_height,
        // pe2.y));
    ...
    private static Vector2dc[] simpleOutLine(List<Vector2dc> points, double width) {
        ...
        Vector2dc[] ret = new Vector2dc[size];
        if (size < 2) {
            return new Vector2dc[0];
        }
        {
            Vector2dc p1 = points.get(0);
            Vector2dc p2 = points.get(1);

            Vector2d v1 = new Vector2d(p2).sub(p1);
            Vector2d n1 = new Vector2d(-v1.y(), v1.x()).normalize().mul(width);
        ...
        for (int i = 1; i < size - 1; i++) {
            ...
            Vector2d v1 = new Vector2d(p2).sub(p1).normalize();

            Vector2d v2 = new Vector2d(p3).sub(p2).normalize();

            Vector2d n1 = new Vector2d(-v1.y, v1.x).mul(width);
            Vector2d n2 = new Vector2d(-v2.y, v2.x).mul(width);
